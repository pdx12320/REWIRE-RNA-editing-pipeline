...
def best_match(seq, target):
    ...
    for i in range(0, max(0, len(seq) - k + 1)):
        ...
        if len(kmer) != k or 'N' in kmer:
            continue
        d = sum(a != b for a, b in zip(kmer, target))
...
def motif_annotation(ref, chrom, strand, pos1, target, flank, max_mismatch):
    pos0 = int(pos1) - 1
    ...
    d, pos, kmer = best_match(seq, target)
    if d == '':
        return {'best_puf_hamming': '', 'best_puf_rel_start': '', 'best_puf_kmer': '', 'pass_puf_like': ''}
    return {
        'best_puf_hamming': d,
        'best_puf_rel_start': str(int(pos) - center),
        'best_puf_kmer': kmer,
        'pass_puf_like': str(int(d) <= max_mismatch),
    }


def build_control_db(control_file, db_path):
    con = sqlite3.connect(db_path)
    cur = con.cursor()
    cur.execute('PRAGMA journal_mode=OFF')
    cur.execute('PRAGMA synchronous=OFF')
    cur.execute('CREATE TABLE control (chrom TEXT, strand TEXT, pos INTEGER, edited INTEGER, total INTEGER, rate REAL, PRIMARY KEY (chrom, strand, pos))')
    batch = []
    n = 0
    with open_text(control_file, 'rt') as f:
        reader = csv.DictReader(f, delimiter='\t')
        for r in reader:
            batch.append((r['chrom'], r['strand'], to_int(r['genomic_pos_1based']), to_int(r['edited_count']), to_int(r['total_count']), to_float(r['edit_rate'])))
            n += 1
            if len(batch) >= 200000:
...
def main():
    ...
    fields = ['sample_id','editor','puf_target_seq','chrom','strand','genomic_pos_1based','transcript_base','genomic_ref_base','edited_base_in_genome','treated_edited_count','treated_total_count','treated_edit_rate','control_edited_count','control_total_count','control_edit_rate','delta_edit_rate','corrected_edit_rate','treated_unedited_count','control_unedited_count','A_count','C_count','G_count','T_count','N_count','gene_names','gene_ids','transcript_ids']
    motif_fields = ['best_puf_hamming','best_puf_rel_start','best_puf_kmer','pass_puf_like'] if ref else []

    n_read = n_merged = n_kept = 0
    with open_text(args.treated, 'rt') as fin, open_text(args.out, 'wt') as fout:
        reader = csv.DictReader(fin, delimiter='\t')
        writer = csv.DictWriter(fout, fieldnames=fields + motif_fields, delimiter='\t')
        writer.writeheader()
        for r in reader:
            n_read += 1
            chrom = r['chrom']
            ...
            delta = t_rate - c_rate
            corrected = max(0.0, min(1.0, delta / (1.0 - c_rate))) if c_rate < 1 else 0.0
            if t_total < args.min_treated_coverage or c_total < args.min_control_coverage:
                continue
            if t_rate < args.min_treated_rate or c_rate > args.max_control_rate:
                continue
            if delta < args.min_delta or corrected < args.min_corrected_rate:
                continue
            out = {
    ...
    if not args.tmp_db:
        try:
            ...
            pass
    print(f'Done. read={n_read} merged={n_merged} candidates={n_kept}', flush=True)

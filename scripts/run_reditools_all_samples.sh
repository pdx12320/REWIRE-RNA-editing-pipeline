...
while IFS=$'\t' read -r sample group replicate srr; do
  ...
  # Accept complete coverage made by an earlier version and add a completion marker.
  ...
  # A previous MPI run may have finished all intervals but failed during the
  # upstream final sorting step. Recover and merge it before rerunning hours of work.
  ...
  # Even when upstream exits non-zero because it mishandles GL/KI version
  # suffixes during final sorting, all interval files can already be complete.
